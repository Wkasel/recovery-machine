    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Core service pricing that customers will see.
          </CardDescription>
        </CardHeader>
          ...
          {getSetting('base_session_price') && (
            <div className="space-y-2">
              <Label htmlFor="base_session_price" className="flex items-center gap-2">
                ...
                {getSetting('base_session_price')?.is_required && <span className="text-red-500">*</span>}
              </Label>
              ...
              {localValues['base_session_price'] !== undefined && (
                <Button 
                  size="sm" 
                  onClick={() => handleSave('base_session_price')}
                  disabled={loading}
                >
                  Save
                </Button>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Pricing for travel and service area limits.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          {/* Travel Fee Per Mile */}
          {getSetting('travel_fee_per_mile') && (
            <div className="space-y-2">
              ...
              {localValues['travel_fee_per_mile'] !== undefined && (
                <Button 
                  size="sm" 
                  onClick={() => handleSave('travel_fee_per_mile')}
                  disabled={loading}
                >
                  Save
                </Button>
          ...
          {getSetting('max_travel_distance') && (
            <div className="space-y-2">
              <Label htmlFor="max_travel_distance" className="flex items-center gap-2">
                ...
                {getSetting('max_travel_distance')?.is_required && <span className="text-red-500">*</span>}
              </Label>
              <div className="relative">
                <Input
                  ...
                  placeholder="25"
                />
                <span className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500">miles</span>
              </div>
              <p className="text-xs text-gray-500">
              ...
              {localValues['max_travel_distance'] !== undefined && (
                <Button 
                  size="sm" 
                  onClick={() => handleSave('max_travel_distance')}
                  disabled={loading}
                >
                  Save
                </Button>
      ...
      <Card className="lg:col-span-2">
        <CardHeader>
          ...
          <CardDescription>
            See how your pricing will appear to customers.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="bg-gray-50 p-6 rounded-lg">
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between text-gray-600">
                <span>Travel (10 miles × ${formatCurrency(getValue('travel_fee_per_mile') || 200)}/mile)</span>
                <span>${formatCurrency((getValue('travel_fee_per_mile') || 200) * 10)}</span>
              </div>
              <div className="border-t pt-2 flex justify-between font-semibold">
                <span>Total</span>
                <span>${formatCurrency((getValue('base_session_price') || 15000) + ((getValue('travel_fee_per_mile') || 200) * 10))}</span>
              </div>
            </div>
          </div>
        </CardContent>
