    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          {getSetting("booking_advance_days") && (
            <div className="space-y-3">
              <Label className="flex items-center gap-2">
                ...
                {getSetting("booking_advance_days")?.is_required && (
              ...
              {localValues["booking_advance_days"] !== undefined && (
                <Button
                  size="sm"
                  onClick={async () => handleSave("booking_advance_days")}
                  disabled={loading}
                >
                  Save
                </Button>
          ...
          {getSetting("booking_buffer_minutes") && (
            <div className="space-y-3">
              ...
              {localValues["booking_buffer_minutes"] !== undefined && (
                <Button
                  size="sm"
                  onClick={async () => handleSave("booking_buffer_minutes")}
                  disabled={loading}
                >
                  Save
                </Button>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Set policies for cancellations and rescheduling.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          {/* Cancellation Hours */}
          {getSetting("booking_cancellation_hours") && (
            <div className="space-y-3">
              <Label className="flex items-center gap-2">
                Cancellation Policy
                {getSetting("booking_cancellation_hours")?.is_required && (
                  <span className="text-red-500">*</span>
                )}
              </Label>
              <div className="space-y-2">
                ...
                <div className="flex justify-between text-sm text-muted-foreground">
                  ...
                  <span className="font-medium">
                    {getValue("booking_cancellation_hours") >= 24
                      ? formatTimeUnit(
                          Math.floor((getValue("booking_cancellation_hours") || 24) / 24),
                          "day"
                        )
              ...
              {localValues["booking_cancellation_hours"] !== undefined && (
                <Button
                  size="sm"
                  onClick={async () => handleSave("booking_cancellation_hours")}
                  disabled={loading}
                >
                  Save
                </Button>
          ...
          {getSetting("booking_auto_confirm") && (
            <div className="space-y-3">
              ...
              {localValues["booking_auto_confirm"] !== undefined && (
                <Button
                  size="sm"
                  onClick={async () => handleSave("booking_auto_confirm")}
                  disabled={loading}
                >
                  Save
                </Button>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Set limits on daily bookings and capacity.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          {getSetting("booking_max_per_day") && (
            <div className="space-y-3">
              ...
              {localValues["booking_max_per_day"] !== undefined && (
                <Button
                  size="sm"
                  onClick={async () => handleSave("booking_max_per_day")}
                  disabled={loading}
                >
                  Save
                </Button>
