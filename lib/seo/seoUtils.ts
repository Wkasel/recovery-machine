...
export function generateOGImageUrl(title?: string, description?: string): string {
  ...
  return params.toString() ? `${baseUrl}?${params.toString()}` : `${siteMetadata.siteUrl}/og-image.jpg`;
}
...
export function generatePageKeywords(
  ...
  const locationKeywords = location ? [`${service || 'wellness'} ${location}`, `mobile spa ${location}`] : [];
...
export function generateLocationUrls(service: string): string[] {
  ...
  return locations.map(location => 
    `/services/${generateSlug(service)}/${location}`
  );
}
