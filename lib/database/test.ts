...
export async function runDatabaseTests() {
  ...
  try {
    ...
    if (reviewsResult.success) {
      console.log(
        `✅ Featured reviews query successful (${reviewsResult.data?.length || 0} reviews)`
      );
  ...
  try {
    ...
    if (availabilityResult.success) {
      console.log(
        `✅ Availability query successful (${availabilityResult.data?.length || 0} slots)`
      );
  ...
  for (const table of tables) {
    try {
      const { data, error } = await import("@/lib/supabase/client").then((m) =>
        m.createBrowserSupabaseClient().from(table).select("count", { count: "exact", head: true })
      );
...
export async function testMigrationStatus() {
  ...
  try {
    const supabase = await import("@/lib/supabase/server").then(async (m) =>
