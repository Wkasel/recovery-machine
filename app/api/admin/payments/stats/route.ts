...
export async function GET(request: NextRequest) {
  try {
    ...
    const totalRevenue =
      totalRevenueResult.data?.reduce((sum, order) => sum + order.amount, 0) || 0;
    const totalOrders = totalOrdersResult.count || 0;
    const activeSubscriptions = activeSubscriptionsResult.data?.length || 0;
    const monthlyRecurring =
      monthlyRecurringResult.data?.reduce((sum, order) => sum + order.amount, 0) || 0;
    const refundedAmount =
      refundDataResult.data?.reduce((sum, order) => sum + order.amount, 0) || 0;
